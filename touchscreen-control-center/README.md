...
状态数据每秒从本机 `zwrt-datad /state` 读取；散热、Wi-Fi 和刷新操作只向 `/control` 发送固定 action 与受约束参数。Mihomo 操作只调用 `/data/mihomo-manager/mihomo-manager.sh` 的固定子命令。
...
仓库不含任何 ZTE 二进制、共享库、字体或图像。上传/下载箭头由本项目运行时生成；信号图标只引用设备已有 `/usr/ui/skin` 路径。
